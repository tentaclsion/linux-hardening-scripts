...
rule-5.1.8()
{
    print_rule_banner "Ensure cron is restricted to authorized users"
    rm -f /etc/cron.deny
...
rule-5.1.9()
{
    print_rule_banner "Ensure at is restricted to authorized users"
    rm -f /etc/at.deny
    touch /etc/at.allow
...
rule-5.2.4()
{
    ...
    if [ -z "${log_lvl_param}" ]; then
        # Select a default parameter for the user.
        log_lvl_param="INFO"
    fi
...
#5.2.13 Ensure only approved MAC algorithms are used (Automated)
rule-5.2.13()
{
...
rule-5.2.15()
{
    ...
    if grep -q "^ClientAliveCountMax\b" $SSHD_CONF; then
        sed -i "s/^ClientAliveCountMax\b.*$/ClientAliveCountMax 3/" $SSHD_CONF
    else
...
rule-5.2.22()
{
    ...
    local banner="Ensure SSH MaxSessions is limited"
    print_rule_banner ${banner}
...
rule-5.4.4()
{
    ...
    # Here, we only set umask in /etc/bash.bashrc. However, if it is set
    # anywhere else, we update it to the correct value. Lastly, if the
    # value is set in /etc/login.defs, make sure the entry in pam.d exists.
    ...
    grep -q "^UMASK\b" $LOGINDEFS_FILE
    if [ $? -eq 0 ]; then
        sed -E -i "s/^UMASK\s+.*/${umask_conf}/" $LOGINDEFS_FILE
        grep -q "pam_umask.so" /etc/pam.d/common-session
        if [ $? -eq 0 ]; then
            sed -E -i "s/.*pam_umask.so.*/session optional			pam_umask.so/" /etc/pam.d/common-session
        else
            echo "session optional			pam_umask.so" >> /etc/pam.d/common-session
        fi
    fi

    grep -q "^umask\b" $PROFILE_FILE
    if [ $? -eq 0 ]; then
        sed -E -i "s/^umask\s+.*/${umask_cmd}/" $PROFILE_FILE
    fi

    grep -q "^umask\b" /etc/profile.d/*
    if [ $? -eq 0 ]; then
        sed -E -i "s/^umask\s+.*/${umask_cmd}/" /etc/profile.d/*
    fi
}
...
{
    local -A rulehash
    local common="5.1.1 5.1.2 5.1.3 5.1.4 5.1.5 5.1.6 5.1.7 5.1.8 5.1.9 5.2.1\
        5.2.2 5.2.3 5.2.4 5.2.6 5.2.7 5.2.8 5.2.9 5.2.10 5.2.11 5.2.12 5.2.13\
        5.2.14 5.2.15 5.2.16 5.2.17 5.2.18 5.2.19 5.2.21 5.2.22 5.3.1 5.3.2\
        5.3.3 5.3.4 5.4.1.1 5.4.1.2 5.4.1.3 5.4.1.4 5.4.1.5 5.4.2 5.4.3 5.4.4\
        5.4.5 5.5 5.6"
    rulehash[lvl1_server]="$common"
    rulehash[lvl2_server]="${rulehash[lvl1_server]}"" 5.2.5 5.2.20"
    rulehash[lvl1_workstation]="$common"" 5.2.5"
    rulehash[lvl2_workstation]="${rulehash[lvl1_workstation]}"" 5.2.20"
