...
#
#
#Prep Ubuntu 20.04 vanilla install for CIS 20.04 benchmark audit run
#This script will prepare a 20.04 Ubuntu installed system to
...
{
    if [ $1 != "custom" ]; then
        echo "***Applying Level-${1:3:1} scored ${1:5} remediation for failures on a fresh Ubuntu 20.04 install***"
    else
        echo "***Applying custom selection of remediation rules for failures on a fresh Ubuntu 20.04 install***"
    fi
}
